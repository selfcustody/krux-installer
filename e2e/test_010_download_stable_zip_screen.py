...
class TestDownloadStableZipScreen(GraphicUnitTest):
    ...
    def test_init(self, mock_get_running_app):
        ...
        self.assertEqual(
            grid.children[1].id, "download_stable_zip_screen_label_progress"
        )
        self.assertEqual(grid.children[0].id, "download_stable_zip_screen_label_info")
    ...
    def test_on_progress(self, mock_get_running_app):
        ...
        # default assertions
        with patch(
            "src.app.screens.download_stable_zip_screen.DownloadStableZipScreen.downloader"
        ) as mock_downloader:
            mock_downloader.downloaded_len = 8
            mock_downloader.content_len = file.getbuffer().nbytes

            # pylint: disable=no-member
            DownloadStableZipScreen.on_progress(data=bytes(file.read(8)))
            self.assertEqual(
                screen.ids["download_stable_zip_screen_label_progress"].text, text
            )
    ...
    def test_on_progress_done(self, mock_sleep, mock_get_running_app):
        ...
        with patch(
            ...
            DownloadStableZipScreen.on_progress(data=bytes(file.read(8)))
            self.assertEqual(
                screen.ids["download_stable_zip_screen_label_progress"].text,
                text_progress,
            )
            self.assertEqual(
                screen.ids["download_stable_zip_screen_label_info"].text, text_label
            )
