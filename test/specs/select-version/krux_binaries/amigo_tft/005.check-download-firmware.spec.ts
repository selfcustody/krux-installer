...
describe('check downloaded \'odudex/krux_binaries/raw/main/maixpy_amigo_tft/firmware.bin\' on file system', () => {
  ...
  before(async () => {
    ...
    firmware = join(api.documents, name, 'odudex', 'krux_binaries', 'raw', 'main', 'maixpy_amigo_tft', 'firmware.bin') 
  })
  ...
  it('should downloaded firmware file on disk have correct size', async () => {
    ...
    expectChai(bytes).to.be.equal('1.74 MB')
  })
})
