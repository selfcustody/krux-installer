...
describe('SelectVersionPage: download \'odudex/krux_binaries/raw/main/maixpy_bit/firmware.bin\'', () => {
  ...
  // eslint-disable-next-line no-undef
  it('should card subtitle be \'device: maixpy_bit\'', async () => {  
    await DownloadTestFirmware.cardSubtitle.waitForExist()
    await expectWDIO(DownloadTestFirmware.cardSubtitle).toHaveText('device: maixpy_bit')
  })
