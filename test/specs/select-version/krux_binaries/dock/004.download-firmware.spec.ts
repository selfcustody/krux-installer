...
describe('SelectVersionPage: download \'odudex/krux_binaries/raw/main/maixpy_dock/firmware.bin\'', () => {
  ...
  // eslint-disable-next-line no-undef
  it('should card subtitle be \'device: maixpy_dock\'', async () => {  
    await DownloadTestFirmware.cardSubtitle.waitForExist()
    await expectWDIO(DownloadTestFirmware.cardSubtitle).toHaveText('device: maixpy_dock')
  })
