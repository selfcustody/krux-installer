...
PYPROJECT_DICT = { "tool": { "poetry": { "name": "test", "version": "1", "description": "Hello World" } } }
...
class TestConstants(TestCase):
    ...
    @patch('src.utils.constants.get_name', return_value=PYPROJECT_DICT["tool"]["poetry"]["name"])
    ...
    @patch('src.utils.constants.get_version', return_value=PYPROJECT_DICT["tool"]["poetry"]["version"])
    ...
    @patch('src.utils.constants.get_description', return_value=PYPROJECT_DICT["tool"]["poetry"]["description"])
    ...
    def test_007_ensure_get_correct_description(self):
        ...
        self.assertEqual(description, "A GUI based application to flash Krux firmware on K210 based devices")
