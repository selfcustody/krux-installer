...
export const config = {
    ...
    onWorkerStart: function (cid: string, caps: any, specs: any, args: any, execArgv: any) {
      ...
      return new Promise<void>(function(resolve, reject) {
        if (specs[0].indexOf('000.config.spec.ts') !== -1) {
          ...
          setTimeout(function() {
            let config = ''
            if (process.platform === 'linux') {
              config = join(process.env.HOME as string, '.config', 'krux-installer', 'config.json')
            } else if (process.platform === 'win32') {
              config = join(process.env.APPDATA as string, 'krux-installer', 'config.json')
            } else if (process.platform === 'darwin') {
              config = join(process.env.HOME as string, 'Library', 'Application Support', 'krux-installer', 'config.json')
            }
